        ...
        <div class="md:w-1/2 p-5">
           <div class="shadow bg-white p-5 mb-5">
            @auth
                ...
                @if (session('mensaje'))
                    <div class="bg-green-500 text-white mb-6 rounded-lg text-sm p-2 text-center font-bold">
                            {{ session('mensaje') }}
                    </div>
                ...
                <form action="{{ route('comentarios.store', ['post' => $post,'user' => $user]) }}" method="POST">
                    @csrf
                    <div class="mb-5">
                        <label for="comentario" class="mb-2 block uppercase text-gray-500 font-bold sha">
